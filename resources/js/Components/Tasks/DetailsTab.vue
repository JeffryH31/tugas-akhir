...
const addLabel = (label) => {
  ...
  router.post(
    ...
    { label_id: label.id },
    {
      ...
      onError: () => {
        props.localTask.labels = props.localTask.labels.filter(
          (l) => l.id !== label.id
        );
    ...
    { assignee_ids: ids },
    {
      ...
      onError: () => {
        if (isAssigned) props.task.assignees.push(member);
          ...
          props.task.assignees = props.task.assignees.filter(
            (a) => a.id !== userId
          );
      },
    }
  ...
  <div class="pa-5">
    ...
    <div class="section-card">
        ...
        <div class="prop-value">
          <v-menu :disabled="!canOperateTasks">
            <template v-slot:activator="{ props: menuProps }">
              <v-btn
                v-bind="menuProps"
                :color="currentPriority?.color || 'grey'"
                variant="tonal"
                size="small"
                class="text-none"
              >
                <v-icon start size="14"
                  >mdi-flag{{ currentPriority ? "" : "-outline" }}</v-icon
                >
      ...
      <div class="prop-row">
        ...
        <div class="prop-value">
          <div class="d-flex align-center ga-2 flex-wrap">
            ...
            <span
              v-if="!displayedAssignees.length"
              ...
              <v-card color="surface" min-width="240">
                <v-list density="compact">
                  ...
                  >
                    <template v-slot:prepend>
                      <v-avatar
                        :color="member.avatar_color || 'primary'"
                        size="24"
                        class="mr-2"
                      >
      ...
      <div class="prop-row">
        ...
        <div class="prop-value">
          <div class="d-flex align-center ga-1 flex-wrap">
            <v-chip
              v-for="label in localTask.labels"
              :key="label.id"
              :color="label.color"
              size="small"
              variant="flat"
              :closable="canOperateTasks"
                ...
                >
                  ...
                  <v-list-item v-if="!workspaceLabels.length" disabled>
                    <v-list-item-title class="text-body-2 text-grey"
                      >No labels yet</v-list-item-title
                    >
                  </v-list-item>
                </v-list>
      ...
      <template v-if="isSubtask">
        ...
        <div class="prop-row">
          <div class="prop-label">
            <v-icon size="16" class="prop-icon"
          ...
          <div class="prop-value">
            <v-menu :disabled="!canManageTaskStructure">
              ...
              <v-card color="surface" min-width="260">
                <v-list density="compact">
                  <v-list-item
                    :active="!localTask.sprint_id"
                    @click="changeSprint(null)"
                  >
                    <template v-slot:prepend>
                    ...
                    <v-list-item-title
                      >Backlog (Not in sprint)</v-list-item-title
                    >
                    <template v-slot:append>
                      <v-icon
                        v-if="!localTask.sprint_id"
                        size="16"
                        color="primary"
                        >mdi-check</v-icon
                      >
                  ...
                  <v-list-item
                    ...
                    :key="sprint.id"
                    :active="sprint.id === localTask.sprint_id"
                    @click="changeSprint(sprint.id)"
                  >
                    <template v-slot:prepend>
                      <v-icon
                        size="16"
                        :color="isSprintActive(sprint) ? 'success' : 'primary'"
                      >
                        {{
                    ...
                    <template v-slot:append>
                      <v-icon
                        v-if="sprint.id === localTask.sprint_id"
                        size="16"
                        color="primary"
                        >mdi-check</v-icon
                      >
          ...
          <div class="prop-value">
            <v-menu location="bottom start" :disabled="!canManageTaskStructure">
              ...
              <v-card color="surface" min-width="280">
                <v-list density="compact">
                  <v-list-item
                    :active="!completionTargetStatusIdState"
                    @click="setCompletionAutomation(null)"
                  >
                    <template v-slot:prepend>
                      <span
                        class="automation-option-dot automation-option-dot--off"
                      />
                    </template>
                    <v-list-item-title>Off</v-list-item-title>
                    <template v-slot:append>
                      <v-icon
                        v-if="!completionTargetStatusIdState"
                        size="16"
                        color="primary"
                        >mdi-check</v-icon
                      >
                  ...
                  <v-list-item
                    v-for="status in completionStatusOptions"
                    :key="status.id"
                  ...
                  >
                    ...
                    <template v-slot:append>
                      <v-icon
                        v-if="status.id === completionTargetStatusIdState"
                        size="16"
                        color="primary"
                        >mdi-check</v-icon
                      >
      ...
      <div class="prop-row">
        ...
        <div class="prop-value">
          <v-menu
            v-model="showTimeEstimatePicker"
            :close-on-content-click="false"
            :disabled="!canOperateTasks"
          >
            <template v-slot:activator="{ props: menuProps }">
            ...
            <v-card color="surface" min-width="280">
              ...
              <v-card-actions>
                ...
                <v-btn
                  size="small"
                  variant="text"
                  @click="showTimeEstimatePicker = false"
                  >Cancel</v-btn
                >
                <v-btn
                  size="small"
        ...
        <div class="prop-row">
          ...
          <div class="prop-value d-flex align-center ga-2 flex-wrap">
            ...
            <v-chip
              ...
              size="x-small"
              :color="
                localTask.schedule_variance_minutes > 0 ? 'warning' : 'success'
              "
              variant="tonal"
            >
        ...
        <div class="prop-row">
          ...
          <div class="prop-value">
            <div class="d-flex align-center ga-2">
              ...
              <v-btn
                v-if="!isTracking && canOperateTasks"
                icon="mdi-play"
                color="success"
                variant="tonal"
                size="x-small"
        ...
        <div class="prop-value">
          <div class="spent-progress-wrap">
            ...
            <v-progress-linear
              v-if="localTask.time_estimate"
              :model-value="
                (localTask.time_spent / localTask.time_estimate) * 100
              "
              :color="
                localTask.time_spent > localTask.time_estimate
                  ? 'error'
                  : 'primary'
              "
              height="4"
              rounded
              class="spent-progress-bar mt-1"
            />
          </div>
        </div>
      </div>
            ...
            <template v-if="(localTask.checklist_total ?? 0) > 0">
              <div class="d-flex align-center ga-3">
                <v-progress-linear
                  ...
                  :color="
                    (localTask.progress ?? 0) >= 100 ? 'success' : 'primary'
                  "
                  height="5"
                  rounded
      ...
      <template v-if="isSubtask">
        ...
        <div class="prop-row prop-row--top">
          ...
          <div class="prop-value">
            <div class="d-flex flex-wrap ga-1 align-center">
              <v-chip
                v-for="dep in localTask?.dependencies || []"
                :key="dep.id"
                size="small"
                color="warning"
                variant="tonal"
              ...
              <v-menu v-if="canOperateTasks" :close-on-content-click="false">
                ...
                <v-card color="surface" min-width="260">
                  ...
                  >
                    <v-list-item
                      v-for="s in availableForDependency"
                      :key="s.id"
                      :disabled="dependencyLoading"
                    ...
                    <v-list-item
                      v-if="availableForDependency.length === 0"
                      disabled
                    >
                      <v-list-item-title class="text-body-2 text-grey">
            ...
            <div
              v-if="!(localTask?.dependencies || []).length"
        ...
        <div class="prop-row prop-row--top">
          ...
          <div class="prop-value">
            <div class="d-flex flex-wrap ga-1 align-center">
              <v-chip
                v-for="dep in localTask?.dependents || []"
                :key="'s-' + dep.id"
                size="small"
                color="error"
                variant="tonal"
            ...
            <div
              v-if="!(localTask?.dependents || []).length"
              class="text-caption text-grey mt-1"
            >
    ...
    <div v-if="!isSubtask" class="section-card mt-4">
      ...
      <div v-if="topLevelSubtasks.length" class="cu-subtask-list">
        <div
          ...
          :key="sub.id"
          class="cu-subtask-row"
          @click="$emit('open-subtask', sub)"
        >
          <div
            class="cu-subtask-checkbox"
            @click.stop="canOperateTasks && toggleSubtask(sub)"
          >
            <span
              class="cu-checkbox-box"
              :class="{
                'cu-checkbox-box--done': sub.completed_at,
              }"
              :style="{
                '--status-color': sub.status?.color || '#6b7280',
              }"
            >
              <v-icon v-if="sub.completed_at" size="14" color="white"
                >mdi-check</v-icon
              >
            </span>
          </div>
          ...
          <span
            ...
            :class="{ 'cu-subtask-name--done': sub.completed_at }"
          >
            {{ sub.name }}
          </span>
          ...
          <div class="cu-subtask-meta">
            <span
              ...
              :class="{
                'cu-meta-date--overdue':
                  new Date(sub.due_date) < new Date() && !sub.completed_at,
              }"
            >
            ...
            <v-icon
              v-if="sub.priority_level || sub.priority"
              size="16"
              :color="
                PRIORITY_MAP[sub.priority_level]?.color ||
                sub.priority?.color ||
                'grey'
              "
              >mdi-flag</v-icon
            ...
            <v-avatar
              ...
              :color="sub.assignees[0].avatar_color || 'primary'"
              size="24"
              class="cu-meta-avatar"
            >
              <span class="cu-avatar-text">{{
                sub.assignees[0].initials
              }}</span>
            </v-avatar>

            <span
              class="cu-meta-status"
              :style="{ color: sub.status?.color || '#6b7280' }"
            >
              {{ sub.status?.name || "Open" }}
            </span>
          </div>
        </div>
      </div>
    ...
    >
      ...
      <div v-if="(localTask.children ?? []).length" class="cu-subtask-list">
        <div
          ...
          :key="child.id"
          class="cu-subtask-row"
          @click="$emit('open-subtask', child)"
        >
          <div
            class="cu-subtask-checkbox"
            @click.stop="canOperateTasks && toggleChildSubtask(child)"
          >
            <span
              class="cu-checkbox-box"
              :class="{
                'cu-checkbox-box--done': child.completed_at,
              }"
              :style="{
                '--status-color': child.status?.color || '#6b7280',
              }"
            >
              <v-icon v-if="child.completed_at" size="14" color="white"
                >mdi-check</v-icon
              >
            </span>
          </div>
          ...
          <span
            class="cu-subtask-name"
            :class="{ 'cu-subtask-name--done': child.completed_at }"
          >
            {{ child.name }}
          </span>
          ...
          <div class="cu-subtask-meta">
            <span
              ...
              :class="{
                'cu-meta-date--overdue':
                  new Date(child.due_date) < new Date() && !child.completed_at,
              }"
            >
              <v-icon size="13">mdi-calendar-blank-outline</v-icon>
              {{
                new Date(child.due_date).toLocaleDateString("en-US", {
                  month: "short",
                  day: "numeric",
                })
              }}
            ...
            <v-icon
              v-if="child.priority_level || child.priority"
              size="16"
              :color="
                PRIORITY_MAP[child.priority_level]?.color ||
                child.priority?.color ||
                'grey'
              "
              >mdi-flag</v-icon
            >
            ...
            <v-avatar
              ...
              :color="child.assignees[0].avatar_color || 'primary'"
              size="24"
              class="cu-meta-avatar"
            >
              <span class="cu-avatar-text">{{
            ...
            <span
              class="cu-meta-status"
              :style="{ color: child.status?.color || '#6b7280' }"
            >
              {{ child.status?.name || "Open" }}
            </span>
          </div>
        </div>
      </div>
    ...
    <v-dialog
      v-if="canManageTaskStructure"
      v-model="showLabelEditor"
      max-width="420"
    >
      <v-card>
        <v-card-title class="d-flex align-center ga-2">
          <v-icon :color="editingLabel ? 'warning' : 'primary'"
