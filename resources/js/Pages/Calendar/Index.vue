...
const getWeekBars = (days) => {
    ...
    const candidates = filteredSubtasks.value
        ...
        .filter(({ range }) => rangesOverlap(range.start, range.end, weekStartDate, weekEndDate))
        .sort((a, b) => {
    ...
    candidates.forEach(({ subtask, range }) => {
        const visualStart = range.start < weekStartDate ? weekStartDate : range.start;
...
const filteredSubtasks = computed(() => {
    return props.subtasks.filter(subtask => {
        const matchesSearch = !searchQuery.value ||
            subtask.name.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
...
const completedSubtasks = computed(() => filteredSubtasks.value.filter(s => s.completed_at).length);
const overdueSubtasks = computed(() => {
    const today = new Date().toISOString().split('T')[0];
    return filteredSubtasks.value.filter(s => !s.completed_at && s.due_date && s.due_date < today).length;
});
...
<template>
    <MainLayout>
        ...
        <div class="h-full flex gap-1 bg-[#1e1e1e]">
            <!-- Sidebar -->
            <div class="w-72 border-r border-[#2d2d30] flex flex-col overflow-y-auto bg-[#1e1e1e]">
                <!-- Mini Calendar Navigation -->
                <div class="p-4 border-b border-[#2d2d30]">
                    <div class="flex items-center justify-between mb-3">
                        <h3 class="text-sm font-semibold text-gray-300">{{ year }}</h3>
                        <div class="flex gap-1">
                            <v-btn size="x-small" icon @click="previousMonth" density="compact">
                                <v-icon size="16">mdi-chevron-left</v-icon>
                            </v-btn>
                            <v-btn size="x-small" icon @click="nextMonth" density="compact">
                    ...
                    <div class="mini-calendar">
                        ...
                        <div class="grid grid-cols-7 gap-1">
                            <div v-for="day in calendarDays.slice(0, 42)" :key="`mini-${day.date.getTime()}`"
                                class="p-1 text-center text-xs rounded cursor-pointer transition" :class="{
                                    'bg-[#4c9aff] text-white font-semibold': isToday(day.date),
                                    'text-gray-500 opacity-50': !day.isCurrentMonth,
                                    'text-gray-300 hover:bg-[#2d2d30]': day.isCurrentMonth && !isToday(day.date),
                ...
                <!-- Filters -->
                <div class="p-4 border-b border-[#2d2d30]">
                    <h3 class="text-sm font-semibold text-gray-300 mb-3">Filters</h3>

                    <!-- Search -->
                    <div class="mb-3">
                        <v-text-field v-model="searchQuery" density="compact" placeholder="Search tasks..."
                    ...
                    <div class="mb-3">
                        ...
                        <v-select v-model="filterAssignee" :items="assigneeOptions" item-title="name" item-value="id"
                            variant="outlined" density="compact" placeholder="All assignees" clearable hide-details />
                    </div>

                    <!-- Priority Filter -->
                    <div class="mb-3">
                        <label class="text-xs text-gray-500 block mb-1">Priority</label>
                        <v-select v-model="filterPriority" :items="priorityOptions" item-title="label" item-value="id"
                            variant="outlined" density="compact" placeholder="All priorities" clearable hide-details />
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <v-select v-model="filterStatus" :items="statusOptions" item-title="name" item-value="id"
                            variant="outlined" density="compact" placeholder="All statuses" clearable hide-details />
                    </div>

                    <!-- Clear Filters -->
                    <v-btn v-if="searchQuery || filterAssignee || filterPriority || filterStatus" size="small"
                        variant="text" density="compact"
                        @click="() => { searchQuery = ''; filterAssignee = null; filterPriority = null; filterStatus = null; }"
                        class="w-full">
                ...
                <!-- Summary Stats -->
                <div class="p-4 border-b border-[#2d2d30]">
                    <h3 class="text-sm font-semibold text-gray-300 mb-3">Summary</h3>
                    <div class="space-y-2 text-sm">
                        <div class="flex justify-between text-gray-400">
                            <span>Total Tasks</span>
                            <span class="font-semibold">{{ totalSubtasks }}</span>
                        </div>
                        <div v-if="completedSubtasks > 0" class="flex justify-between text-green-400">
                            <span>Completed</span>
                            <span class="font-semibold">{{ completedSubtasks }}</span>
                        </div>
                        <div v-if="overdueSubtasks > 0" class="flex justify-between text-red-400">
                            <span>Overdue</span>
                            <span class="font-semibold">{{ overdueSubtasks }}</span>
                        </div>
                        <div class="pt-2 border-t border-[#2d2d30] mt-2">
                            <div class="flex justify-between text-gray-400 mb-1">
                                <span>Progress</span>
                                <span class="font-semibold">{{ totalSubtasks > 0 ? Math.round((completedSubtasks /
                                    totalSubtasks) * 100) : 0 }}%</span>
                            </div>
                            <v-progress-linear
                                :value="totalSubtasks > 0 ? (completedSubtasks / totalSubtasks) * 100 : 0" height="4" />
                        </div>
                    </div>
                </div>
                ...
                <div class="p-4 flex-1 overflow-y-auto">
                    <h3 class="text-sm font-semibold text-gray-300 mb-3">Upcoming</h3>
                    <div class="space-y-2">
                        <div v-for="item in getAgendaItems" :key="item.id"
                            class="p-2 rounded bg-[#252526] hover:bg-[#2d2d30] cursor-pointer transition text-xs"
                            @click="handleTaskClick(item)">
                            <div class="font-semibold text-gray-300 truncate">{{ item.name }}</div>
                            <div class="text-gray-500 text-xs">
                                {{ item.dueDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) }}
                            </div>
                        </div>
                        <div v-if="getAgendaItems.length === 0" class="text-center text-gray-500 py-4">
                            No upcoming tasks
                        </div>
                    </div>
                </div>
            ...
            <div class="flex-1 flex flex-col bg-[#1e1e1e] overflow-hidden">
                ...
                <div class="flex items-center justify-between p-4 border-b border-[#2d2d30]">
                    <div class="flex items-center gap-4">
                        ...
                        <div class="flex items-center gap-2">
                            <v-btn-group density="compact" variant="outlined" divided>
                                <v-btn @click="previousPeriod" size="small" icon>
                                    <v-icon>mdi-chevron-left</v-icon>
                                </v-btn>
                                <v-btn @click="goToToday" size="small" min-width="70">Today</v-btn>
                    ...
                    <div class="flex items-center gap-2">
                        <v-btn-group density="compact" variant="outlined" divided>
                            <v-btn :variant="viewMode === 'month' ? 'flat' : 'outlined'"
                                @click="viewMode = 'month'; loadCalendarData()" size="small" title="Month view">
                                <v-icon size="18">mdi-calendar-month</v-icon>
                            </v-btn>
                            <v-btn :variant="viewMode === 'week' ? 'flat' : 'outlined'"
                                @click="viewMode = 'week'; loadCalendarData()" size="small" title="Week view">
                                <v-icon size="18">mdi-calendar-week</v-icon>
                            </v-btn>
                            <v-btn :variant="viewMode === 'agenda' ? 'flat' : 'outlined'" @click="viewMode = 'agenda'"
                                size="small" title="Agenda view">
                ...
                <div class="flex-1 overflow-auto">
                    ...
                    <div v-if="viewMode === 'month'" class="p-4">
                        <div class="calendar-month-grid">
                            <!-- Day headers -->
                            <div v-for="day in ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday']"
                                :key="day" class="calendar-header">
                                <span class="hidden sm:inline">{{ day }}</span>
                                <span class="sm:hidden">{{ day.slice(0, 3) }}</span>
                            </div>

                            <div v-for="(weekRow, weekIndex) in calendarWeeks" :key="`week-${weekIndex}`" class="calendar-week-row">
                                <div class="week-bars-overlay"
                                    :style="{ gridTemplateRows: `repeat(${getWeekLaneCount(weekRow, 3)}, 22px)` }">
                                    <div v-for="bar in getWeekBarsLimited(weekRow, 3)" :key="`bar-${weekIndex}-${bar.subtask.id}-${bar.row}`"
                                        ...
                                        <span class="span-name">{{ bar.subtask.name }}</span>
                                        <v-avatar v-if="bar.subtask.assignee" size="16"
                                            :color="bar.subtask.assignee.avatar_color || 'primary'">
                                            <span class="text-[10px]">{{ bar.subtask.assignee.initials }}</span>
                                        </v-avatar>
                                        <v-icon v-if="bar.subtask.completed_at" size="12">mdi-check</v-icon>
                                        <v-icon v-if="bar.endsAfterWeek" size="12">mdi-chevron-right</v-icon>
                                    </div>
                                    <div v-if="getWeekBarsOverflowCount(weekRow, 3) > 0" class="week-bar-overflow">
                                        +{{ getWeekBarsOverflowCount(weekRow, 3) }} more ongoing
                                    </div>
                                </div>

                                <div class="week-days-grid">
                                    <div v-for="(day, dayIndex) in weekRow" :key="`day-${weekIndex}-${dayIndex}`"
                                        class="calendar-day" :class="{
                                            'current-month': day.isCurrentMonth,
                                            'other-month': !day.isCurrentMonth,
                                            'today': isToday(day.date)
                                        }" @dragover="handleDragOver" @drop="handleDrop(day.date, $event)">
                                        <div class="day-header">
                                            <span class="day-number">{{ day.date.getDate() }}</span>
                                        </div>

                                        <div class="day-content">
                                            <div v-for="subtask in getVisibleSubtasksForDate(day.date, 1)"
                                                :key="subtask.id" class="calendar-task" draggable
                                                @dragstart="handleDragStart(subtask, $event)"
                                                @click.stop="handleTaskClick(subtask)">
                                                <v-chip size="small" :color="getStatusColor(subtask.status)" class="w-full text-left">
                                                    <div class="flex items-center gap-1 w-full overflow-hidden">
                                                        <span class="text-xs truncate flex-1">{{ subtask.name }}</span>
                                                        <v-icon v-if="subtask.completed_at" size="10">mdi-check-circle</v-icon>
                                                    </div>
                                                </v-chip>
                                            </div>

                                            <div v-if="getOverflowSubtasksCount(day.date, 1) > 0"
                                                class="calendar-overflow">
                                                +{{ getOverflowSubtasksCount(day.date, 1) }} more
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div v-else-if="viewMode === 'week'" class="week-view p-4">
                        ...
                        <div class="mini-calendar-grid week-grid">
                            ...
                            <div v-for="(day, index) in weekDays" :key="`wkday-${index}`" class="calendar-day" :class="{
                                'current-month': day.isCurrentMonth,
                                'other-month': !day.isCurrentMonth,
                                'today': isToday(day.date)
                            }" @dragover="handleDragOver" @drop="handleDrop(day.date, $event)">
                    ...
                    <div v-else-if="viewMode === 'agenda'" class="p-4">
                        <div v-if="filteredSubtasks.length === 0" class="text-center text-gray-500 py-8">
                            <v-icon size="48" class="mb-4">mdi-calendar-blank-outline</v-icon>
                            <p>No tasks found</p>
                        </div>
                        <div v-else class="space-y-2 max-w-4xl">
                            <div v-for="subtask in filteredSubtasks.sort((a, b) => {
                                ...
                                return aDate - bDate;
                            })" :key="subtask.id" class="agenda-item">
                                <div class="flex gap-3 p-3 rounded hover:bg-[#252526] cursor-pointer transition"
                                    ...
                                    <div class="flex-shrink-0">
                                        <div class="w-10 h-10 rounded flex items-center justify-center"
                                            ...
                                            <v-icon v-if="subtask.completed_at" size="16">mdi-check</v-icon>
                                            <v-icon v-else size="16">mdi-circle-outline</v-icon>
                                        </div>
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <div class="font-semibold text-white">{{ subtask.name }}</div>
                                        <div class="text-sm text-gray-400">{{ subtask.task?.name }}</div>
                                        <div class="flex items-center gap-2 mt-2 flex-wrap">
                                            <v-chip size="x-small" v-if="subtask.assignee">
                                                <v-icon start size="12">mdi-account</v-icon>
                                                {{ subtask.assignee.name }}
                                            </v-chip>
                                            <v-chip size="x-small" v-if="subtask.due_date">
                                            ...
                                            <v-chip size="x-small" v-if="subtask.time_spent">
                                                <v-icon start size="12">mdi-clock</v-icon>
                                                {{ Math.floor(subtask.time_spent / 60) }}h
                                            </v-chip>
                                        </div>
                                    </div>
                                </div>
        ...
        <!-- Task Detail Panel -->
        <TaskDetailPanel v-model="showTaskDetail" :task="selectedTask" :workspace="workspace"
            :space="selectedTask?.task?.task_list?.space" :list="selectedTask?.task?.task_list"
            :parent-task="selectedTask?.task" :statuses="workspace.spaces?.flatMap(s => s.statuses) || []"
            :members="workspace.members || []" :labels="workspace.labels || []" />
    </MainLayout>
</template>
